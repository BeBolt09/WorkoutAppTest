...
const ThirdScreen = ({ route, navigation }) => {
  ...
  const exerciseCards = listOfExercises.split("\n").map((exercise, index) => {
    ...
    const textDecorationStyle = /^[-*\d]/.test(exercise)
      ? {}
      : ThirdScreenStyles.itemText;
    const itemStyle = isSelected ? ThirdScreenStyles.selectedItem : null;
    const arrowContainerStyle = isSelected
      ? [
          ThirdScreenStyles.selectedArrowContainer,
          ThirdScreenStyles.selectedArrowColor,
        ]
      : ThirdScreenStyles.arrowContainer;

    return (
      <TouchableOpacity
        key={index}
        onPress={() => handleExerciseSelection(cleanedExercise)}
      >
        <View style={[ThirdScreenStyles.item, itemStyle]}>
          <Text
            style={[ThirdScreenStyles.itemText, textDecorationStyle]}
            numberOfLines={1}
          >
            {exercise}
          </Text>
          <View style={arrowContainerStyle}>
            <Entypo
              ...
              size={18}
              color={isSelected ? "#e7ebed" : "#b8bac1"}
            />
          </View>
        </View>
      </TouchableOpacity>
          ...
          <View style={ThirdScreenStyles.body}>
            <ScrollView style={ThirdScreenStyles.scrollView}>
              <Text style={ThirdScreenStyles.h2}>
                {exerciseCards.length} Substitutes for{" "}
                <Text style={ThirdScreenStyles.inputValue}>{inputValue}</Text>
              </Text>
              {exerciseCards}
            </ScrollView>
          </View>
